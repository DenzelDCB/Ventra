...
function Home() {
  ...
  const handleSignUp = async (e) => {
    ...
    if (emailRegex.test(email) === false) return showError("Email does not represent a real email.")
  ...
  return (
    <div>
      ...
      {lignip === 1 && email && password && age && (<div style={{border: '1px solid black', borderRadius: '8px', padding: '8px', margin: '8px', width: '400px',}}><DiceCaptcha onVerify={setCaptchaValid} /></div>)}
      <br />
      {lignip === 1 && (
        <button onClick={handleSignUp} disabled={!captchaValid} style={{ padding: '8px', borderRadius: '8px', border: '1px solid black', margin: '5px', cursor: 'pointer', }}>
          Sign Up
        </button>
      )}
      {lignip === 0 && (
        <button onClick={handleSignIn} style={{ padding: '8px', borderRadius: '8px', border: '1px solid black', margin: '5px', cursor: 'pointer', }}>
          Log In
        </button>
      )}
